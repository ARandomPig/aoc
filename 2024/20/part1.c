...
/*   Updated: 2024/12/20 15:16:22 by tomoron          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
long int check_around(uint32_t **dists, t_pos pos, int len, int height, int cheat_dist, int cur_dist)
{
	long int res;
	int x;
	int y;
...
long int count_cheats(char **map, t_pos pos, uint32_t **dists)
{
	long int res;
	uint32_t cur_dist;
	int len;
	...
	while(map[pos.y][pos.x] != 'E')
	{
		...
		if(cur_dist != (uint32_t)-1)
			res += check_around(dists, pos, len, height, 2, cur_dist);
